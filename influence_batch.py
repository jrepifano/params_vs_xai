...
def influence(train_data, train_labels, test_data, test_labels, model, layer_weight, btchsz=20000, n=1, std=0.2,
    ...
    for itr in range(n):
        ...
        for itr, (train_data_batch, train_labels_batch) in enumerate(trainloader):
            ...
            train_loss = criterion(train_pred, train_labels_batch.to('cuda:1'))
            total_train_loss += train_loss

        total_test_loss = 0
        for itr, (test_data_batch, test_labels_batch) in enumerate(testloader):
            test_pred = model(test_data_batch)
            test_loss = criterion(test_pred, test_labels_batch.to('cuda:1'))
        ...
        for itr, (train_data_batch, train_labels_batch) in enumerate(trainloader):
            ...
            x_out = model(train_data_batch)
            x_loss = criterion(x_out, train_labels_batch.to('cuda:1'))
